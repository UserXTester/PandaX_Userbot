...
LOGS.info(f"PandaX_Userbot Version - {petercordpanda_version}")

VC_PANDA = udB.get("VC_SESSION") or Var.VC_SESSION
API_PANDA = Var.API_ID
API_HASHPANDA = Var.API_HASH
PLUGINS = dict(
    root="PandaX_Userbot",
    include=[
        "vc." + udB.get("PLUGIN"): udB.set("PLUGIN", "player"),
        "ping",
        "sysinfo"
    ]
)
