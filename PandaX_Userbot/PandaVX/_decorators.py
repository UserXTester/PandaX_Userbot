...
from PandaX_v20 import PandaX_version as ult_ver
...
from . import owner_and_sudos, should_allow_sudo, sudoers, petercordpanda_bot
...
def ilhammansiz_cmd(allow_sudo=should_allow_sudo(), **args):
    ...
    def decorator(func):
        ...
        def doit(mode):
            async def wrapper(ult):
                ...
                try:
                    ...
                    ftext = "**PandaX_Userbot  Client Error:** `Forward this to` @TEAMSquadUserbotSupport\n\n"
                    ftext += "`PandaX_Userbot Version: " + str(pyver)
                    ftext += "\nPandaX_Userbot Version: " + str(ult_ver)
                    ftext += "\nTelethon Version: " + str(telever) + "\n\n"
                    ftext += "--------START  PandaX_Userbot CRASH LOG--------"
                    ftext += "\nDate: " + date
        ...
        if "PandaX_ASsistant" in type:
            args["pattern"] = compile_pattern(pattern, "/")
            asst.add_event_handler(doit("PandaX_ASsistant"), events.NewMessage(**args))
