...
from ..Panda.core import LIST, LOADED
...
from . import owner_and_sudos, should_allow_sudo, petercordpanda_bot
...
def ilhammansiz_cmd(allow_sudo=should_allow_sudo(), **args):
    ...
    def decorator(func):
            ...
            async def wrapper(ult):
                ...
                try:
                    ...
                    await asst.send_message(
                        ...
                        buttons=[
                            Button.url("Hubungi", "t.me/diemmmmmmmmmm"),
                            Button.url(
                                "Repl",
                                "https://replit.com/@ilhammansiz/UserbotSessionTEAM#userbotString.py",
                            ),
                        ],
                    )
                    exit()
                    ...
                    ftext = "**Panda Client Error:** `Forward this to` @TeamSquadUserbotSupport\n\n"
                    ftext += "**PandaX_Userbot Version:** `" + str(pyver)
                    ftext += "`\n**Panda Version:** `" + str(ult_ver)
                    ftext += "`\n**Telethon Version:** `" + str(telever) + "`\n\n"
                    ftext += "--------START PANDA CRASH LOG--------"
                    ftext += "\n**Date:** `" + date
                    ...
                    if len(ftext) > 4096:
                        ...
                        await asst.send_file(
                            int(udB["LOG_CHANNEL"]),
                            "logs.txt",
                            caption="**PandaX_Userbot Client Error:** `Forward this to` @TeamSquadUserbotSupport\n\n",
                        )
                        os.remove("logs.txt")
                    else:
                        await asst.send_message(
        ...
        if "official" in type:
            args["outgoing"] = True
            petercordpanda_bot.add_event_handler(doit("official"), events.NewMessage(**args))
            if udB.get("TAKE_EDITS"):
                args["func"] = lambda x: not (
                    isinstance(x.chat, types.Channel) and x.chat.broadcast
                )
                petercordpanda_bot.add_event_handler(
                    doit("official"),
            ...
            if allow_sudo:
                ...
                petercordpanda_bot.add_event_handler(doit("sudo"), events.NewMessage(**args))
                del args["outgoing"]
