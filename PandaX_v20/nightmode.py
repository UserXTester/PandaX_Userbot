...
from PandaX_Userbot.functions.night_db import *
...
@ilhammansiz_cmd(pattern="nmtime ?(.*)")
...
@ilhammansiz_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = night_grps()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await petercordpanda_bot.get_entity(x)
            if ok.username:
                name += "@" + ok.username
            else:
                name += ok.title
...
async def open_grp():
    ...
    for chat in chats:
        try:
            ...
            await petercordpanda_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
